...
class Pedido
{
    ...
    public function crearPedido($productos)
    {
        ...
        foreach ($productos as $producto) {
            $item = $this->insertarProductoPedido($idPedido, $producto['nombre'], $this->estado);
            $this-> preciototal += $item['precio'];
        }


        // Insertar el pedido en la tabla 'pedidos'
        $consulta = $objAccesoDatos->prepararConsulta("INSERT INTO pedidos (id_pedido, cliente_asignado, estado, preciototal, mesa_asignada, mozo_asignado, tiempo_entrega) 
                                                    VALUES (:id_pedido, :cliente_asignado, :estado, :preciototal, :mesa_asignada, :mozo_asignado, :tiempo_entrega)");
        ...
        $consulta->bindValue(':tiempo_entrega', $this->tiempo_entrega, PDO::PARAM_INT);  // Agregado el tiempo de entrega
        $consulta->execute();
    ...
    private function insertarProductoPedido($idPedido, $nombreProducto, $estado)
    {
        $objAccesoDatos = AccesoDatos::obtenerInstancia();
        ...
        if ($producto) {
            $consulta = $objAccesoDatos->prepararConsulta("INSERT INTO productos_pedidos (nombre, id, id_pedido, estado,sector) 
                                                            VALUES (:nombreProducto, :producto, :id_pedido, :estado,:sector)");
            
            $consulta->bindValue(':nombreProducto', $nombreProducto, PDO::PARAM_STR);  // Corregido el nombre
            $consulta->bindValue(':producto', $producto['id'], PDO::PARAM_INT);  // Corregido el nombre y tipo
            $consulta->bindValue(':id_pedido', $idPedido, PDO::PARAM_STR);
